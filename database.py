    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS ai_error_logs (
                ...
                tags_content TEXT,  -- JSON 数组，原始 Tags
                error_msg TEXT,
...
async def get_push_source(illust_id: int) -> str | None:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT source FROM push_history WHERE illust_id = ?", (illust_id,)
        ) as cursor:
            row = await cursor.fetchone()
...
async def update_xp_tag_pairs(pairs: list[tuple[str, str, float]]):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.executemany(
            "INSERT INTO xp_tag_pairs (tag1, tag2, weight) VALUES (?, ?, ?)", pairs
        )
        await db.commit()
...
async def update_ai_error_status(error_id: int, status: str):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE ai_error_logs SET status = ? WHERE id = ?", (status, error_id)
        )
        await db.commit()
...
async def get_push_stats(days: int = 7) -> dict:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # 推送总数
        cursor = await db.execute(
            "SELECT COUNT(*) FROM push_history WHERE pushed_at > ?", (since,)
        )
        row = await cursor.fetchone()
...
async def format_stats_report(days: int = 7) -> str:
    ...
    if stats["top_artists"]:
        artists_str = "\n".join(
            f"  - ID {a[0]}: {a[1]}张" for a in stats["top_artists"][:3]
        )
    else:
        artists_str = "  暂无数据"
...
async def reset_xp_data():
    """
    重置所有 XP 分析数据（适用于 Prompt 变更后需要重新清洗的情况）
    将会清除：
    1. XP 画像 (xp_profile, xp_tag_pairs)
...
async def unblock_tag(tag: str) -> bool:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        # 1. 移除手动屏蔽
        cursor = await db.execute("DELETE FROM blocked_tags WHERE tag = ?", (tag,))
        manual_deleted = cursor.rowcount > 0

        # 2. 重置厌恶计数 (针对自动屏蔽)
        cursor = await db.execute(
            "UPDATE tag_feedback_stats SET dislike_count = 0 WHERE tag = ?", (tag,)
        )
...
async def is_tag_blocked(tag: str) -> bool:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "SELECT 1 FROM blocked_tags WHERE tag = ?", (tag.lower().strip(),)
        )
        return await cursor.fetchone() is not None
...
async def get_artist_score(artist_id: int) -> float:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "SELECT score FROM artist_profile WHERE artist_id = ?", (artist_id,)
        )
        row = await cursor.fetchone()
...
async def get_top_xp_tags(limit: int = 15) -> list[tuple[str, float]]:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "SELECT tag, weight FROM xp_profile ORDER BY weight DESC LIMIT ?", (limit,)
        )
        rows = await cursor.fetchall()
        return [(row[0], row[1]) for row in rows]
...
async def get_illust_embedding(illust_id: int) -> list[float] | None:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "SELECT embedding FROM illust_embeddings WHERE illust_id = ?", (illust_id,)
        )
        row = await cursor.fetchone()
        if row and row[0]:
            return json.loads(row[0])
...
async def save_user_embedding(
    user_id: int, embedding: list[float], model: str, profile_hash: str
):
    """保存用户画像 Embedding"""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
...
async def cleanup_old_embeddings(days: int = 60) -> int:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "DELETE FROM illust_embeddings WHERE created_at < ?", (cutoff,)
        )
        await db.commit()
        return cursor.rowcount
...
async def get_bookmark_count(user_id: int = None) -> int:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        if user_id:
            cursor = await db.execute(
                "SELECT COUNT(*) FROM xp_bookmarks WHERE user_id = ?", (user_id,)
            )
        else:
...
async def save_batch_mapping(message_id: int, chat_id: str, illusts: list):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        data = [
            (message_id, str(chat_id), i + 1, illust.id)
