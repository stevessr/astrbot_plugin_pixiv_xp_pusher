...
class AITagProcessor:
    ...
    async def process_tags(self, tags: list[str]) -> tuple[list[str], dict[str, str]]:
        ...
        if uncached:
            ...
            logger.info(
                f"发现 {len(uncached)} 个新 Tag，开始 AI 处理 ({self.batch_size}/批)..."
            )
    ...
    async def _process_single_batch(self, tags: list[str]):
        ...
        for attempt in range(max_logic_retries):
            try:
                logger.info(
                    f"🤖 正在请求 AI 清洗 (尝试 {attempt + 1}/{max_logic_retries})..."
                )
...
def _build_ai_prompt(tags: list[str]) -> str:
    ...
    for canonical, query_str in TAG_TRANSLATIONS.items():
        aliases = [
            p.strip().replace("(", "").replace(")", "") for p in query_str.split(" OR ")
        ]
...
### 1. 过滤（放入 meaningless）
删除以下类型的标签：
- **平台/统计类**: original, pixiv, users 入り，bookmark, 収藏，仕事絵
...
class XPProfiler:
    ...
    async def load_blacklist(self):
        ...
        try:
            ...
            logger.info(
                f"已加载黑名单：{len(blocked_tags)} 个手动屏蔽 Tag + {len(blocked_artists)} 个屏蔽画师"
            )
        except Exception as e:
    ...
    async def build_profile(
        self, user_id: int, scan_limit: int = 500, include_private: bool = False
    ) -> dict[str, float]:
        """
        扫描收藏，构建 XP 权重字典
        ...
        async def fetch_segment(is_private):
            ...
            head_items = await self.client.get_bookmarks(
                ...
                on_batch=make_callback(
                    update_cursor_key=cursor_key if mode == "slow" else None
                ),
            )
            ...
            if do_tail_resume:
                logger.info(
                    f"[{desc}] ⚡ 触发高效断点续传，直接跳转到：{saved_cursor[:60]}..."
                )
        ...
        logger.info(f"XP 分析数据源：{len(bookmarks)} 个收藏作品 (含本地历史)")

        # 统计 Tag 出现次数和时间 (存储 illust_id 用于正确计算 DF)
        # 支持权重系数 (liked items = 0.5x，因为 apply_feedback 已给过 1.0x)
        tag_occurrences: dict[str, list[tuple[int, datetime, float]]] = defaultdict(
            list
        )
        ...
        for illust in bookmarks:
            ...
            for tag in illust.tags:
                ...
                if normalized and normalized not in self.stop_words:
                    tag_occurrences[normalized].append(
                        (illust.id, illust.create_date, weight_mult)
                    )

        # AI 处理：过滤无意义 tag 和归类同义 tag
        if self.ai_processor.enabled:
            ...
            logger.info(f"AI 处理后剩余 {len(tag_occurrences)} 个有效 Tag")
        ...
        if saturated_tags:
            logger.info(
                f"🎯 饱和度检测：{len(saturated_tags)} 个高频 Tag 自动加入停用词"
            )
            for tag, sat in saturated_tags[:5]:  # 只显示前 5 个
        ...
        if len(bookmarks) < cold_start_threshold:
            logger.info(
                f"🧊 检测到冷启动场景 (收藏：{len(bookmarks)} < {cold_start_threshold})"
            )
            try:
                ...
                for tag, freq in popular_tags:
                    ...
                    if (
                        normalized_tag
                        and normalized_tag not in profile
                        and normalized_tag not in self.stop_words
                    ):
        ...
        logger.info(
            f"构建 XP 画像完成，共 {len(profile)} 个 Tag，{len(pairs_to_save)} 个热门组合"
        )
        return profile

    # 预编译正则：去除 users 入り 后缀
    ...
    async def apply_feedback(self, illust: Illust, action: str, config: dict):
        ...
        for tag in illust.tags:
            ...
            if action == "like":
                ...
                if count == dislike_threshold:
                    logger.info(
                        f"Tag '{normalized}' 累计否认 {count} 次，建议加入黑名单"
                    )
                    # 只记录第一个达到阈值的 Tag
                    ...
                    logger.debug(
                        f"Tag '{normalized}' 权重 -{adjusted_penalty:.2f} (分级惩罚)"
                    )
        ...
        if illust.user_id:
            try:
                ...
                logger.debug(
                    f"画师 {illust.user_id} ({illust.user_name}) 权重 {artist_delta:+.1f}"
                )
            except Exception as e:
