...
class Illust:
    ...
    view_count: int
    page_count: int
    image_urls: list[str]  # 所有页的原图 URL
    is_r18: bool
    ai_type: int  # 0=非 AI, 1=辅助 AI, 2=纯 AI
...
class PixivClient:
    ...
    async def login(self) -> bool:
        """
        登录（可选）
        无 token 时使用游客模式（功能受限）
        """
        if not self.refresh_token:
            logger.warning("未配置 Token，使用游客模式（仅支持公开内容）")
            return False
    ...
    async def get_bookmarks(
        ...
        Args:
            ...
            limit: 获取数量
            private: 是否获取私密收藏
            stop_ids: 遇到这些 ID 时停止获取（用于增量更新）
            skip_ids: 遇到这些 ID 时跳过但继续（用于笨拙的断点续传）
            on_batch: 每批次回调 (items, next_url)
            start_url: 起始 URL（用于高效断点续传）
        """
        ...
        while len(illusts) < limit:
            ...
            if not result.get("illusts"):
                if "error" in result:
                    logger.error(f"API 错误：{result['error']}")
                elif len(illusts) == 0 and skipped_count == 0 and not start_url:
                    logger.warning(
                        f"用户 {user_id} 未返回任何收藏 (restrict={restrict})。请确认账号是否有对应权限或收藏。"
                    )
                break
            ...
            if page_count % 5 == 0:
                logger.info(
                    f"进度：已获取 {len(illusts)} 新收藏 (已跳过 {skipped_count} 旧收藏)..."
                )
        ...
        logger.info(
            f"获取结束：共获取 {len(illusts)} 个新收藏 (跳过 {skipped_count} 个)"
        )
        return illusts
    ...
    async def get_related_illusts(
        self, illust_id: int, limit: int = 30
    ) -> list[Illust]:
        """
        ...
        while len(illusts) < limit and page < max_pages:
            async with self.rate_limiter:
                if next_qs:
                    ...
                    filtered_qs = {
                        k: v for k, v in next_qs.items() if k in supported_keys
                    }
    ...
    async def get_ranking(
        self, mode: str = "day", date: str | None = None, limit: int = 50
    ) -> list[Illust]:
        """
        获取排行榜
            ...
            while True:
                async with self.rate_limiter:
                    if next_qs:
                        ...
                        result = await self.api.user_following(
                            user_id, restrict=restrict
                        )
    ...
    async def download_image(self, url: str) -> bytes:
        ...
        return await download_image_with_referer(
            self._session, url, self.download_semaphore, proxy=self.proxy_url
        )
