...
def calculate_match_score(
    ...
    for tag in illust.tags:
        ...
        # Fallback: 尝试原始 Tag 的小写 (有些特例可能未被归一化覆盖)
    ...
    # 匹配数量奖励：log(1 + n) / log(6) → 匹配 5 个以上趋于饱和
...
class ContentFilter:
    ...
    async def filter(
        ...
        if self.min_create_days > 0:
            time_threshold = datetime.now(
                illusts[0].create_date.tzinfo if illusts else None
            ) - timedelta(days=self.min_create_days)
        ...
        if filtered_by_time > 0:
            logger.debug(
                f"过滤 {filtered_by_time} 个超过 {self.min_create_days} 天的老图"
            )
                ...
                if cached and cached[1] == profile_hash:
                    ...
                    if user_embedding:
                        await db.save_user_embedding(
                            user_id, user_embedding, self.embedder.model, profile_hash
                        )
                        logger.info("已更新用户画像 Embedding")

                # 批量获取作品 Embedding 缓存
                illust_ids = [ill.id for ill in unique_result]
                illust_embeddings_cache = await db.get_illust_embeddings_batch(
                    illust_ids
                )
                logger.debug(
                    f"Embedding 缓存命中：{len(illust_embeddings_cache)}/{len(illust_ids)}"
                )
            ...
            try:
                ...
                for i, (illust, tag_score) in enumerate(uncached_embeddings):
                    ...
                    if emb:
                        ...
                        score = (
                            1 - semantic_weight
                        ) * tag_score + semantic_weight * semantic_score
                    else:
                        score = tag_score
        ...
        if scored_result:
            ...
            def sort_key(item):
                ...
                base_score = score * self.match_weight + normalized_bookmark * (
                    1 - self.match_weight
                )
            ...
            try:
                ...
                if len(candidates_for_ai) > 5:  # 至少需要一定数量才有意义
                    ...
                    ai_scores = await self.ai_scorer.score_candidates(
                        candidates_for_ai, xp_profile, recent_likes, recent_dislikes
                    )
                    ...
                    if ai_scores:
                        ...
                        # 重新排序
                        scored_result = [
                            (ill, score_map.get(ill.id, 0)) for ill in sorted_illusts
                        ]
        ...
        if self.diversity_enabled:
            ...
            for illust, score in scored_result:
                ...
                multiplier = (1.0 - self.diversity_floor) * (
                    self.diversity_decay**pos
        ...
        logger.info(
            f"过滤后剩余 {len(final_result)} 个作品 (涉及 {len(artist_count)} 个画师)"
        )
        return final_result
    ...
    def check_illust(self, illust: Illust) -> bool:
        ...
        if self.min_create_days > 0:
            ...
            time_threshold = datetime.now(illust.create_date.tzinfo) - timedelta(
                days=self.min_create_days
            )
