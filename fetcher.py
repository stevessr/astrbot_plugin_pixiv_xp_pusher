...
class ContentFetcher:
    ...
    def __init__(
        ...
        self.bookmark_threshold = bookmark_threshold or {
            "search": 1000,
            "subscription": 0,
        }
    ...
    def _adaptive_threshold(
        self, base: int, tag_weight: float, is_combination: bool = False
    ) -> int:
        """
        自适应收藏阈值
    ...
    async def discover(
        self, xp_tags: list[tuple[str, float]], limit: int = 50
    ) -> list[Illust]:
        """
        策略 A：基于 XP 的广泛搜索 (并发优化版)
        """
        if not xp_tags:
            logger.warning("无 XP 标签，跳过搜索")
            return []
        ...
        if remaining > 0:
            ...
            if fallback_tasks:
                logger.info(f"启动 {len(fallback_tasks)} 个单 Tag 补充任务...")
        ...
        logger.info(
            f"XP 搜索获取 {len(filtered_illusts)} 个作品 (原始 {len(all_illusts)})"
        )
    ...
    async def _search_single(self, tag: str, limit: int) -> list[Illust]:
        """单个 Tag 搜索任务"""
        dynamic = await self._get_dynamic_threshold(
            tag, self.bookmark_threshold["search"]
        )
        # 如果是单 Tag 搜索，也给与一定折扣 (0.5)，防止动态阈值过高
            ...
            for artist_id in self.subscribed_artists:
                ...
                try:
                    illusts = await self.sync_client.get_user_illusts(
                        user_id=artist_id, since=since, limit=5
                    )
        ...
        for mode in self.ranking_modes:
            try:
                illusts = await self.client.get_ranking(
                    mode=mode, limit=self.ranking_limit // len(self.ranking_modes)
                )
    ...
    def _weighted_sample(
        self, weighted_tags: list[tuple[str, float]], k: int
    ) -> list[str]:
        """根据权重随机采样 Tag"""
    ...
    async def _get_dynamic_threshold(self, tag: str, base: int) -> int:
        ...
        if tag in self._search_max_bookmarks_cache:
            ...
            try:
                ...
                top_illusts = await self.client.search_illusts(
                    tags=[tag],
                    limit=1,
                    # search_illusts 内部默认是 popular_desc，所以取第 1 个就是 Max Bookmarks 左右
                )
                if top_illusts:
        ...
        relative_threshold = max(
            self.dynamic_threshold_min, int(max_bookmarks * self.dynamic_threshold_rate)
        )
    ...
    async def select_strategies(self, total_limit: int) -> dict[str, int]:
        ...
        logger.info(
            f"MAB 分配：{final_quotas} (Scores: { {k: f'{v:.2f}' for k, v in scores.items()} })"
        )
        return final_quotas
    ...
    async def discover_related(
        ...
        # 3. Fetch Related
        try:
            raw_related = await self.client.get_related_illusts(
                seed_illust_id, limit=limit * 2
            )  # 多抓点备选
        except Exception as e:
    ...
    async def discover_from_engaged_artists(
        self, xp_tags: list[tuple[str, float]], limit: int = 30
    ) -> list[Illust]:
        """
        策略 E：基于互动画师的发现 (Engagement-Based Discovery)
        ...
        try:
            ...
            if not top_artists:
                logger.debug("无互动画师数据，跳过策略 E")
                return []

            logger.info(f"策略 E: 发现 {len(top_artists)} 个互动画师")

            # 2. 并发获取画师最新作品
            from datetime import datetime, timedelta

            since = datetime.now().astimezone() - timedelta(
                days=self.date_range_days * 2
            )  # 扩大时间范围
            ...
            logger.info(
                f"策略 E: 获取 {len(result)} 个作品 (来自 {len(top_artists)} 个互动画师)"
            )

            return result

        except Exception as e:
            logger.error(f"策略 E 执行失败：{e}")
            return []

    async def fetch_content(
        self, xp_tags: list[tuple[str, float]], total_limit: int = 200
    ) -> list[Illust]:
        """
        统一内容获取入口 (MAB 调度)
        ...
        - ranking: 排行榜
        - related: 关联作品发现
        - engagement_artists: 互动画师发现 (策略 E, 新增)
        """
        ...
        if quotas.get("engagement_artists", 0) > 0:
            tasks.append(
                self.discover_from_engaged_artists(
                    xp_tags, limit=quotas["engagement_artists"]
                )
            )
